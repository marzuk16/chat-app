...
Save the agreed implementation plan for issue **#$ARGUMENTS**, create the feature branch, and open a PR.

**Prerequisites:** This command expects `/plan_task $ARGUMENTS` to have already been run in this conversation. If no agreed plan exists in the conversation context, stop and tell the user to run `/plan_task $ARGUMENTS` first.

**If `$ARGUMENTS` is empty or not a number, ask the user for the issue number before proceeding.**

1. Save the agreed plan to `docs/plan/task_plan_$ARGUMENTS.md`. The file should capture: what the feature/fix is, which services and layers are involved, any reusable components, and the proposed tests.

2. Derive a short slug from the feature/fix name (e.g. `add-user-profile`).
...
4. Commit the plan file as the first commit on the branch:
   ```bash
...
7. Add the PR to the **chat-app** GitHub Project and set its fields:
